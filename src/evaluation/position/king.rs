...
fn king_pawn_shelter(position: &Position, king_color: Color, king_square: Square) -> ValueScore {
    ...
    let king_square_rank = king_square.rank() as i8;
    ...
    for file in file_min..=file_max {
        ...
        if our_pawns_on_file.is_empty() {
            shelter_penalty += SHELTER_PENALTY * 2;
            continue;
        }

        let most_advanced_pawn_rank_diff = (match king_color {
            Color::White => {
                our_pawns_on_file.into_iter().next_back().unwrap().rank() as i8 - king_square_rank
            }
            Color::Black => {
                our_pawns_on_file.into_iter().next().unwrap().rank() as i8 - king_square_rank
            }
        })
        .unsigned_abs();

        shelter_penalty += match most_advanced_pawn_rank_diff {
            0 | 1 => 0,
            2 => SHELTER_PENALTY / 2,
            3 | 4 => SHELTER_PENALTY,
            _ => 0,
...
pub fn evaluate_king_safety(position: &Position, midgame_ratio: u8) -> ValueScore {
    ...
    score.saturating_mul(midgame_ratio as ValueScore) / 255
}
...
mod tests {
    ...
    fn broken_shelter_hard() {
        ...
        assert_eq!(position_shelter(&position), SHELTER_PENALTY * 4 + SHELTER_PENALTY);
    }
