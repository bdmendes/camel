...
use crate::position::{board::Piece, square::Square, Color, Position, PositionDiffEntry};
...
impl NeuralNetwork {
    ...
    pub fn evaluate(&mut self, position: &Position) -> i16 {
        if Some(position.zobrist_hash()) == self.last_seen_position.map(|p| p.zobrist_hash()) {
            self.last_result
        } else {
            if let Some(last_position) = self.last_seen_position {
                ...
                for square in Square::list() {
                    if let Some((piece, color)) = position.board.piece_color_at(*square) {
                        self.set(piece, color, *square);
                    }
                }
            }
