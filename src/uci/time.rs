...
fn expected_remaining_moves(position: &Position) -> u32 {
    const TYPICAL_MOVES_PER_GAME: u16 = 60;
    if position.full_move_number > TYPICAL_MOVES_PER_GAME - 20 {
        return 20;
    }
...
fn get_duration_based_on_eval(position: &Position, time: Duration) -> Duration {
    ...
    let cof = 10 + (std::cmp::min(number_available_moves / 4, 10)) as u32;
...
pub fn get_duration(
    ...
    let standard_move_time = if our_duration < Duration::from_millis(500) {
