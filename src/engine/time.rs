...
fn expected_remaining_moves(position: &Position) -> u32 {
    ...
    if position.fullmove_number > TYPICAL_MOVES_PER_GAME - 10 {
        return 10;
    }
    (TYPICAL_MOVES_PER_GAME - position.fullmove_number) as u32
}

fn get_duration_based_on_moves(position: &Position, time: Duration) -> Duration {
    let number_available_moves = position.moves::<false>().len() + 1;
